...
int cancelar_pedido(Fila *fila, Pilha *pilha, Estatisticas_Cancelados *est)
{
    ...
    Pedido *pedido = fila_remover_ultimo(fila);

    if (pedido != NULL)
    {
        printf("Cancelando pedido ID: %d, valor: %.2f\n", pedido->codigoPedido, pedido->valorEstimado);
        push_pilha(pilha, *pedido);
    }
    ...
    {
        printf("Pedido inserido na pilha de cancelados.\n.");
    }
    else
    {
